...
{
    //check for object, add raw JSON text to map keyed by if exists
    ...
    if( do_response > -1 ) {
        ...
        if( d_win.show_response_conditionals ) {
            if( debug_info.find( "condition" ) != debug_info.end() && actual_response.condition ) {
                debug_output.push_back( std::string( "Conditional: [" ) + ( actual_response.condition(
                                            *this ) ? colorize( "true", c_light_green ) : colorize( "false",
                                                    c_light_red ) ) + std::string( "] - " ) + debug_info["condition"] );
            } else {
                debug_output.push_back( "No conditionals" );
            }
        }
        if( debug_info.find( "trial" ) != debug_info.end() ) {
            if( d_win.show_response_conditionals ) {
                if( actual_response.trial.condition ) {
                    debug_output.push_back( std::string( "Trial: [" ) + ( actual_response.trial.condition(
                                                *this ) ? colorize( "true", c_light_green ) : colorize( "false",
                                                        c_light_red ) ) + std::string( "] - " ) + debug_info["trial"] );
                } else {
                    debug_output.push_back( "Trial: " + debug_info["trial"] );
                }
            }
            if( d_win.show_response_effects ) {
                debug_output.push_back( colorize( "Success: ", c_green ) + debug_info["success"] );
                debug_output.push_back( colorize( "Failure: ", c_red ) + debug_info["failure"] );
            }
        }
        if( d_win.show_response_effects ) {
            if( debug_info.find( "effect" ) != debug_info.end() ) {
                debug_output.push_back( "Effect: " + debug_info["effect"] );
            } else {
                debug_output.push_back( "No effects" );
            }
        }
    } else {
        ...
        for( auto eff : speaker_effects ) {
            ...
            if( debug_info.find( "condition" ) != debug_info.end() && d_win.show_dynamic_line_conditionals ) {
                debug_output.push_back( colorize( "CND" + eff_count_str + std::string( ": [" ),
            ...
            if( debug_info.find( "effect" ) != debug_info.end() && d_win.show_dynamic_line_effects ) {
                debug_output.push_back( colorize( "EFF" + eff_count_str + ": " + debug_info["effect"], c_yellow ) );
                added_eff = true;
            }
            eff_count++;
        }
        if( !added_cond && d_win.show_dynamic_line_conditionals ) {
            debug_output.push_back( colorize( "No conditionals", c_yellow ) );
        }
        if( !added_eff && d_win.show_dynamic_line_effects ) {
            debug_output.push_back( colorize( "No effects", c_yellow ) );
        }
    }
